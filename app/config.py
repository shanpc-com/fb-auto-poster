...
class Settings:
    ...
    album_id: str = ""
    album_fallback_to_page: bool = True
...
def load_settings() -> Settings:
    ...
    settings = Settings(
        ...
        album_id=str(get("FB_ALBUM_ID", "")).strip(),
        album_fallback_to_page=_bool(get("ALBUM_FALLBACK_TO_PAGE", True), True),
    )
    ...
    if not settings.post_image: raise ValueError("POST_IMAGE must be true in v7 photo mode.")
    return settings
