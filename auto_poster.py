...
def main():
    ...
    for item in remaining:
        ...
        try:
            ...
            result={"id":"dry-run","post_id":"dry-run","mode":"dry_run"} if s.dry_run else fb.publish_photo_file(caption,image_path)
            log["posted"].append({"keyword":item.original,"display_title":item.display_title,"facebook_post_id":result["post_id"],"facebook_photo_id":result["id"],"publish_mode":result["mode"],"image_url":data.image_url,"image_mode":image_mode,"source_url":data.source_url,"posted_at":datetime.now(timezone.utc).isoformat()})
