...
def build_message(item, data, link, desc, features, tags):
    title = " ".join(item.display_title.split()).strip()
    description = " ".join((desc or "").split()).strip()
    lines = [title, "", description, "", f"Download: {link}", ""]
    details = [
    ...
    shown = [(label, value) for label, value in details if value]
    if shown:
        lines += ["Software information"] + [f"• {label}: {value}" for label, value in shown] + [""]
    if features:
        lines += ["Key features"] + [f"• {' '.join(x.split())}" for x in features[:5] if x.strip()] + [""]
    hashtags = []
    for tag in tags:
        clean = "".join(c for c in tag if c.isalnum())
        if clean and clean.lower() not in {x.lower() for x in hashtags}:
            hashtags.append(clean)
    if hashtags:
        lines.append(" ".join(f"#{tag}" for tag in hashtags[:8]))
    return "\n".join(lines).strip()
...
def main():
    ...
    for item in remaining:
        ...
        try:
            ...
            caption = build_message(item, data, item.site_link or settings.site_link, desc, features, tags)
            print("[POST PREVIEW]\n" + caption)
            image_path, image_mode = prepare_post_image(item.display_title, data.image_url, BASE_DIR / "assets" / "current-post.jpg", settings.request_timeout)
